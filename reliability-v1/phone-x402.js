(function (root, factory) {
  ...
  const METAMASK_DAPP_LINK_BASE = "https://link.metamask.io/dapp/";
  ...
  // MetaMask's documented dapp deeplink takes the HTTPS dapp URL without its
  // scheme as the /dapp/ path. Keep the path, query, and fragment verbatim so
  // the wallet browser reopens this exact invoice rather than reconstructing
  // security-critical fields independently.
  function buildMetaMaskDappLink(paymentPageUrl) {
    const page = new URL(paymentPageUrl);
    requireValue(page.protocol === "https:", "MetaMask handoff requires an HTTPS payment page");
    requireValue(!page.username && !page.password, "MetaMask handoff URL must not contain credentials");
    return METAMASK_DAPP_LINK_BASE + page.host + page.pathname + page.search + page.hash;
  }

  function paymentPageFromMetaMaskDappLink(handoffUrl) {
    const value = String(handoffUrl || "");
    requireValue(value.startsWith(METAMASK_DAPP_LINK_BASE), "unexpected MetaMask handoff origin");
    const target = value.slice(METAMASK_DAPP_LINK_BASE.length);
    requireValue(target.length > 0 && !target.startsWith("/"), "missing MetaMask dapp target");
    return new URL("https://" + target).href;
  }
  ...
  return {
    ...
    METAMASK_DAPP_LINK_BASE: METAMASK_DAPP_LINK_BASE,
    buildMetaMaskDappLink: buildMetaMaskDappLink,
    paymentPageFromMetaMaskDappLink: paymentPageFromMetaMaskDappLink,
